...
import { useWarehouseOrderHistory } from '@/hooks/useOrders'
...
export default function OrderHistoryPage() {
    ...
    const { data: response, isLoading } = useWarehouseOrderHistory(warehouse?._id || '', {
        page,
        limit,
    ...
    return (
        <div className="space-y-6 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Order Audit Trail</h1>
                    <p className="text-slate-500 font-bold mt-1">Detailed history for <span className="text-slate-900">{warehouse?.name}</span></p>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-[40px] p-24 border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-slate-400 font-bold mt-3 max-w-sm leading-relaxed">
                        No order activities have been recorded for this warehouse yet.
                    </p>
                </div>
            ) : (
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {orders.map((order: any) => (
                                    <tr 
                                        key={order._id} 
                                        onClick={() => router.push(`/manager/orders/history/${order._id}`)}
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <Badge variant="outline" className={cn("text-[10px] uppercase font-black px-2 py-0.5 rounded-lg", getStatusColor(order.status))}>
