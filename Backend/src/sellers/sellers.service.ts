...
import { validateGstin, extractStateCode } from '../common/utils/gstin.util';
...
export class SellersService {
  ...
  async create(
    ...
    if (existingSeller) {
      throw new ConflictException(
        'Seller profile already exists for this user or email',
      );
    }

    let slug = slugify(createSellerDto.storeName, {
      lower: true,
      strict: true,
    });
    ...
    // GSTIN Validation
    if (
      createSellerDto.gstNumber &&
      !validateGstin(createSellerDto.gstNumber)
    ) {
    ...
    try {
      ...
      console.error(
        'Failed to create notification for new seller registration:',
        error,
      );
    }
  ...
  private decryptSeller(seller: any): Seller {
    ...
    if (sellerObj.documentPaths) {
      if (sellerObj.documentPaths.aadhar)
        sellerObj.documentPaths.aadhar = decrypt(
          sellerObj.documentPaths.aadhar,
        );
      if (sellerObj.documentPaths.pan)
        sellerObj.documentPaths.pan = decrypt(sellerObj.documentPaths.pan);
      if (sellerObj.documentPaths.license)
        sellerObj.documentPaths.license = decrypt(
          sellerObj.documentPaths.license,
        );
    }
