...
export class AuthService {
  ...
  async verifyOtp(email: string, otp: string) {
    try {
      ...
      if (!cachedOtp || String(cachedOtp) !== String(otp)) {
        this.logger.warn(
          `Invalid OTP attempt for ${email}. Expected: ${cachedOtp}, Received: ${otp}`,
        );
      ...
      this.logger.error(
        `OTP verification error for ${email}: ${error.message}`,
      );
  ...
  async registerSubAdmin(registerDto: RegisterSubAdminDto) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Sub-admin registration failed for ${registerDto.email}: ${error.message}`,
        error.stack,
      );
  ...
  async registerManager(registerDto: RegisterManagerDto) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Manager registration failed for ${registerDto.email}: ${error.message}`,
        error.stack,
      );
  ...
  async resetPassword(resetPasswordDto: ResetPasswordDto) {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Password reset failed for ${resetPasswordDto.email}: ${error.message}`,
      );
