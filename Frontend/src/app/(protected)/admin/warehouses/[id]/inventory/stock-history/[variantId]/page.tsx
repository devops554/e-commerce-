...
const AdminStockHistoryPage = () => {
    const { id: warehouseId, variantId } = useParams() as { id: string, variantId: string }
    ...
    const { data: response, isLoading: isHistoryLoading } = useStockHistory(warehouseId, {
        page,
        limit,
        search: debouncedSearch,
        productId: variantId // In some contexts productId is used for variantId
    })

    const [range, setRange] = useState('1m')
    const { data: statsResponse, isLoading: isStatsLoading } = useStockHistoryStats(warehouseId, {
        range,
        productId: variantId
    })
    ...
    useEffect(() => {
        setBreadcrumbs([
            ...
            { label: 'Warehouse Details', href: `/admin/warehouses/${warehouseId}` },
            { label: 'Stock History' }
        ])
    }, [setBreadcrumbs, warehouseId])

    useEffect(() => {
        setPage(1)
    }, [debouncedSearch])

    if (isHistoryLoading && !response) {
    ...
    return (
        <div className="space-y-6 pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Inventory Audit</h1>
                    <p className="text-slate-500 font-bold mt-1">Movement trail for Variant: <span className="text-slate-900">{variantId}</span></p>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                    ...
                    <ExcelExportButton
                        ...
                        filename={`Stock_History_${warehouseId}_${format(new Date(), 'yyyyMMdd')}`}
                ...
                {isHistoryLoading ? (
                    ...
                    <div className="bg-white rounded-[40px] p-24 border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-slate-400 font-bold mt-3 max-w-sm leading-relaxed">
                            {searchTerm ? "Try searching for something else." : "Once stock movements occur, a detailed audit trail will appear here automatically."}
                        </p>
                    </div>
                ) : (
                    <>
