...
import { getOrderItemGst } from '../common/utils/gst.util';
...
export class ProductsService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      this.logger.log(
        'Initializing ProductsService: Syncing indexes and clearing caches...',
      );
      ...
      if (indexes.some((idx) => idx.name === 'sku_1')) {
        ...
        this.logger.log(
          'Successfully dropped legacy unique index "sku_1" from products collection',
        );
      }
      ...
      const products = await this.productModel
        .find(
          ...
          'title slug brand keywords tags seo shortDescription',
        )
        .exec();
      ...
      this.logger.error(
        `Error in ProductsService onModuleInit: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  private indexProductForAutocomplete(product: {
    ...
    for (const kw of product.seo?.keywords ?? [])
      if (kw) terms.add(kw.toLowerCase());
    ...
    for (const term of terms) {
      this.redisService
        ...
        .catch((err) =>
          this.logger.warn(
            `Autocomplete index failed for "${term}": ${err.message}`,
          ),
        );
    }
  }
    ...
    if (data.seo?.keywords && Array.isArray(data.seo.keywords)) {
      if (!data.keywords || data.keywords.length === 0) {
        ...
        data.keywords = Array.from(
          new Set([...data.keywords, ...data.seo.keywords]),
        );
      }
    }
  }
  ...
  public async invalidateProductCache(productId: string, slug?: string) {
    try {
      ...
      this.logger.warn(
        `Cache invalidation failed for product ${productId}: ${error.message}`,
      );
    }
  }
  ...
  async create(
    ...
    try {
      ...
      // TASK 5: Seller Linking Logic
      if (productData.seller) {
        const seller = await this.sellerModel
          .findById(productData.seller)
          .lean();
        ...
        // Auto-set sellerStateCode
        productData.sellerStateCode =
          seller.stateCode || seller.gstNumber?.slice(0, 2) || '';
      }

      // Role guard: Subadmin can only create products for their own seller profile
      if (userRole === UserRole.SUB_ADMIN && productData.seller) {
        const subadminSeller = await this.sellerModel
          .findOne({ user: new Types.ObjectId(createdBy) })
          .lean();
        if (subadminSeller) {
          productData.seller = subadminSeller._id;
          productData.sellerStateCode = subadminSeller.stateCode || '';
        }
      }
  ...
  async findAll(query: {
    ...
    try {
      ...
      if (category) {
        ...
        const idObj = Types.ObjectId.isValid(idStr)
        ...
        if (idObj) {
          matchIds.push(idObj);
          const childIds =
            await this.categoriesService.getSubCategoryIds(idStr);
          childIds.forEach((id) => {
            matchIds.push(id);
            if (Types.ObjectId.isValid(id))
      ...
      if (minPrice !== undefined || maxPrice !== undefined) {
        ...
        const priceMatchingIds = Array.from(
          new Set(priceMatchingVariants.map((v) => v.product.toString())),
        );
        ...
        if (priceMatchingIds.length === 0) {
          ...
          andConditions.push({
            _id: { $in: priceMatchingIds.map((id) => new Types.ObjectId(id)) },
          });
        }
      }
      ...
      if (isActive === true || String(isActive) === 'true') {
        productsWithVariants = productsWithVariants.filter(
  ...
  async findOne(idOrSlug: string): Promise<any> {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Failed to fetch product ${idOrSlug}: ${error.message}`,
        error.stack,
      );
  ...
  async update(
    ...
    try {
      ...
      if (updateProductDto.title && !updateProductDto.slug) {
        updateData.slug = slugify(updateProductDto.title, {
          lower: true,
          strict: true,
        });
      }
    ...
    try {
      ...
      if (!productExists) {
        throw new NotFoundException(
          `Parent product not found: ${createVariantDto.product}`,
        );
      }
    ...
    try {
        ...
        if (userRole === UserRole.SELLER) {
          ...
          if (!product || product.createdBy.toString() !== userId) {
            throw new BadRequestException(
              'You do not have permission to update this product variant',
            );
          }
        }
      ...
      if (!updatedVariant) {
        throw new InternalServerErrorException(
          'Failed to retrieve updated variant',
        );
      }
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Failed to update variant ${id}: ${error.message}`,
        error.stack,
      );
  ...
  async getProductVariants(productId: string): Promise<any[]> {
    try {
      ...
      const variants = await this.variantModel
        ...
        .exec();
      this.logger.debug(
        `Fetched ${variants.length} variants for product ${productId}`,
      );

      // Attach GST breakdown to each variant (intra-state preview by default)
      const variantsWithGst = variants.map((v) => {
        const variantObj = v.toObject() as any;
        if (product?.gst?.hsnCode && product.gst.gstRate !== undefined) {
          const sellingPrice = v.discountPrice || v.price;
          const breakdown = getOrderItemGst(
            sellingPrice,
            product.gst as any,
            false,
          );
          variantObj.gstBreakdown = breakdown;
        }
        return variantObj;
      });
  ...
  /**
   * Returns a single variant with its full GST breakdown computed.
   * Useful for cart / checkout price detail pages.
   */
  async getVariantWithGst(variantId: string): Promise<any> {
    try {
      this.validateObjectId(variantId, 'variant');
      const variant = await this.variantModel
        .findById(variantId)
        .populate<{ product: Product }>('product', 'gst title')
        .lean()
        .exec();
      ...
      const sellingPrice =
        (variant as any).discountPrice || (variant as any).price;

      let gstBreakdown: any = null;
      if (product?.gst?.hsnCode && product.gst.gstRate !== undefined) {
        gstBreakdown = getOrderItemGst(sellingPrice, product.gst, false);
      }

      return { ...variant, gstBreakdown };
    } catch (error) {
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Failed to fetch variant with GST ${variantId}: ${error.message}`,
        error.stack,
      );
  ...
  async getSuggestions(query: string) {
    try {
      ...
      this.logger.warn(
        `Suggestion fetch failed for "${query}": ${error.message}`,
      );
      return [];
    }
  }
