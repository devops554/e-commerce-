...
/* ─────────────────────────────────────────────────────────────────
   page.tsx  —  /manager/orders/history/[id]
   Fully componentised. All sub-panels are individually reusable.
───────────────────────────────────────────────────────────────── */
...
export default function OrderHistoryDetailPage() {
    ...
    const { data: warehouseData, isLoading: isWarehouseLoading } = useManagerWarehouse()
    ...
    useEffect(() => {
        setBreadcrumbs([
            ...
            { label: 'Order History', href: '/manager/orders/history' },
            { label: 'Snapshot Detail' },
        ])
    }, [setBreadcrumbs])

    const isLoading = isOrderLoading || isShipmentLoading || isWarehouseLoading
    ...
    const warehouseLoc = warehouseData?.location
        || (shipment?.warehouseId?.location)
    ...
    /* pull delivery partner: prioritize shipment data as it's the source of truth for assignments */
    ...
    const warehouse = warehouseData
        ? { ...warehouseData, location: warehouseLoc }
        : (typeof shipment?.warehouseId === 'object' ? { ...shipment.warehouseId, location: warehouseLoc } : null)
                ...
                <header className="ohd-fade ohd-fade-1 flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-sm font-medium mt-1">Status Archive &amp; Audit Log</p>
                        </div>
                    </div>
                        ...
                        <div className="ohd-fade ohd-fade-2 ohd-card bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_16px_rgba(0,0,0,0.03)]">
                            <div className="px-6 pt-5 pb-3">
                                ...
                                <p className="text-slate-400 text-xs font-medium mb-0">
                                    Visualization of order&apos;s path from warehouse to customer.
                                </p>
                            </div>
                    ...
                    <div className="space-y-5">
                        ...
                        <OhdCard className="ohd-fade ohd-fade-5">
                            ...
                            <p className="text-xs text-slate-400 font-medium mb-5">
                                Record of all status transitions.
                            </p>
...
.ohd-status-dot { animation: ohd-pulse 2.2s ease-in-out infinite; }
@keyframes ohd-pulse {
