...
export class CartsController {
  ...
  async addItem(
    ...
    this.logger.log(
      `User ${req.user._id} adding item ${body.variantId} to cart`,
    );
  ...
  async removeItem(@Req() req: any, @Param('variantId') variantId: string) {
    this.logger.log(
      `User ${req.user._id} removing item ${variantId} from cart`,
    );
  ...
  async syncCart(
    ...
    this.logger.log(
      `User ${req.user._id} syncing ${body.items?.length || 0} items to cart`,
    );
