...
import {
    CheckCircle2, Clock, Truck, XCircle, ShoppingBag, BadgeCheck,
} from 'lucide-react'

export type StatusKey = 'created' | 'pending' | 'confirmed' | 'shipped' | 'delivered' | 'failed' | 'cancelled'
...
const TIMELINE_STEPS: { key: string; label: string; icon: React.ReactNode }[] = [
    { key: 'created', label: 'Placed', icon: <ShoppingBag className="w-3.5 h-3.5" /> },
...
export function OrderStatusCard({ orderStatus, totalAmount, createdAt, orderId, cancelReason, cancelAt }: Props) {
    const cfg = ORDER_STATUS_CONFIG[orderStatus as StatusKey] ?? ORDER_STATUS_CONFIG.pending
    const stepIndex = TIMELINE_STEPS.findIndex((s) => s.key === orderStatus)
                ...
                {orderStatus === 'cancelled' && cancelReason && (
                    <div className="mt-5 p-4 rounded-2xl bg-rose-50 border border-rose-100 flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                            <p className="text-xs font-black text-rose-900">Cancellation Details</p>
