...
export const SubcategorySidebar = ({ parentId }: SubcategorySidebarProps) => {
    const params = useParams()
    const currentSlug = params.slug as string
    const [catSearch, setCatSearch] = React.useState('')

    // Fetch subcategories for the sidebar
    const { data, isLoading } = useSubcategories(parentId, {
        isActive: true,
        limit: 50,
    })
    ...
    return (
        <aside className="sticky top-24 self-start w-full pr-4">
            <div className="space-y-6">
                <div>
                    <h2 className="text-xl font-black text-slate-900 mb-4 px-2 tracking-tight">Subcategories</h2>
                    ...
                    <div className="space-y-1">
                        {filteredCategories.length > 0 ? filteredCategories.map((cat) => {
                            ...
                            <div className="py-8 text-center px-2">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">No matching subcategories</p>
                            </div>
                        )}
                    </div>
                </ScrollArea>
