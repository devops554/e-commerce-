...
export default function AdminReturnsDashboard() {
    ...
    // Fetch returns without warehouseId to get all of them across the platform
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                        ...
                        Platform Return Requests
                    </h1>
                    <p className="text-slate-500 font-bold mt-1">
                        Viewing <span className="text-slate-900">All Returns</span> across all warehouses
                    </p>
                </div>
            </div>

            {/* Quick Stats - Currently static approximations, can be wired to actual counts if available in response */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard title="Total Requests" value={data?.total || 0} icon={<Clock className="text-amber-500" />} color="amber" />
                <StatCard title="In Transit" value="-" icon={<Truck className="text-blue-500" />} color="blue" />
                <StatCard title="Awaiting QC" value="-" icon={<Package className="text-purple-500" />} color="purple" />
                <StatCard title="Completed" value="-" icon={<CheckCircle2 className="text-emerald-500" />} color="emerald" />
            </div>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto">
                            ...
                            <Select value={status} onValueChange={setStatus}>
                                ...
                                <SelectContent className="rounded-2xl border-none shadow-2xl">
                                    ...
                                    <SelectItem value="RECEIVED_AT_WAREHOUSE">Received</SelectItem>
                                    <SelectItem value="QC_PASSED">QC Passed</SelectItem>
                                    <SelectItem value="QC_FAILED">QC Failed</SelectItem>
                                    <SelectItem value="REFUND_COMPLETED">Refunded</SelectItem>
                                    ...
                                    data?.data.map((req: any) => (
                                        <TableRow key={req._id} className="hover:bg-slate-50/50 group border-slate-50">
                                            ...
                                            <TableCell className="pr-6 text-right">
                                                <Link href={`/admin/orders/${req.orderId?._id || req.orderId}`}>
