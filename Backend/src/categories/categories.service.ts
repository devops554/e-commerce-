...
export class CategoriesService {
    ...
    private async clearCategoryCache(id?: string, slug?: string) {
        try {
            ...
            this.logger.warn(`Cache clear failed in CategoriesService: ${error.message}`);
        }
    }
    ...
    async create(createCategoryDto: CreateCategoryDto, createdBy: string): Promise<Category> {
        try {
            const slug = createCategoryDto.slug || slugify(createCategoryDto.name, { lower: true, strict: true });
            ...
            if (createCategoryDto.order === undefined) {
                const lastCategory = await this.categoryModel.findOne().sort({ order: -1 }).exec();
    ...
    async findAll(query: {
        ...
        try {
            const { page = 1, limit = 10, search, productType, parentId, isActive, sort } = query;
    ...
    async findOne(idOrSlug: string): Promise<Category> {
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(`Failed to fetch category ${idOrSlug}: ${error.message}`, error.stack);
    ...
    async update(id: string, updateCategoryDto: UpdateCategoryDto, userId: string): Promise<Category> {
        try {
            ...
            if (updateCategoryDto.name && !updateCategoryDto.slug) {
                updateData.slug = slugify(updateCategoryDto.name, { lower: true, strict: true });
            }
    ...
    async remove(id: string): Promise<void> {
        try {
            ...
            await this.categoryModel.updateMany({ parentId: id }, { parentId: null }).exec();
    ...
    async getSubCategoryIds(parentId: string): Promise<string[]> {
        try {
            const children = await this.categoryModel.find({ parentId }).select('_id').exec();
            ...
            this.logger.error(`Failed to fetch subcategories for ${parentId}: ${error.message}`);
            return [];
        }
    }
}
