    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        try {
            ...
            if (payload.type !== 'delivery_partner') {
                throw new UnauthorizedException('Invalid token type for delivery partner');
            }

            const partner = await this.deliveryService.findById(payload.sub);
            if (!partner || partner.accountStatus === 'BLOCKED') {
                throw new UnauthorizedException('Delivery partner account is inactive or not found');
            }
