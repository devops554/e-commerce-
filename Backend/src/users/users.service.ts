import { Injectable, NotFoundException, BadRequestException, InternalServerErrorException, Logger, HttpException } from '@nestjs/common';
...
export class UsersService {
    ...
    async addAddress(userId: string, addressData: any): Promise<User> {
        try {
            ...
            if (addressData._id) {
                // Update existing
                const index = user.addresses.findIndex(a => a._id.toString() === addressData._id);
                if (index !== -1) {
    ...
    async removeAddress(userId: string, addressId: string): Promise<User> {
        try {
            ...
            user.addresses = user.addresses.filter(a => a._id.toString() !== addressId) as any;
            return await user.save();
        } catch (error) {
            if (error instanceof HttpException) throw error;
    ...
    async updateStatus(id: string, status: UserStatus): Promise<User> {
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(`Failed to update user status ${id}: ${error.message}`, error.stack);
