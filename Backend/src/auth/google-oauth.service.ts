...
export class GoogleOAuthService {
    ...
    constructor() {
        ...
        if (!this.clientId || !this.clientSecret || !this.redirectUri) {
            this.logger.error('Google OAuth credentials missing from environment variables');
        }
    }
    ...
    private async exchangeCodeForTokens(code: string): Promise<GoogleTokens> {
        try {
            ...
            if (!response.ok) {
                ...
                throw new BadRequestException('Invalid authorization code or credentials');
            }

            return await response.json();
        } catch (error) {
            if (error instanceof HttpException) throw error;
            this.logger.error(`Error exchanging code for tokens: ${error.message}`, error.stack);
            throw new InternalServerErrorException('Google authentication exchange failed');
        }
    }
    ...
    private async getUserInfoFromGoogle(accessToken: string): Promise<GoogleUserInfo> {
        try {
            ...
            if (!response.ok) {
                this.logger.warn('Failed to get user info from Google (invalid access token)');
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(`Error getting user info from Google: ${error.message}`, error.stack);
    ...
    async verifyIdToken(idToken: string): Promise<GoogleUserInfo> {
        try {
            ...
            if (!response.ok) {
                this.logger.warn(`Invalid ID token verification attempt: ${idToken.substring(0, 10)}...`);
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(`Error verifying Google ID token: ${error.message}`, error.stack);
