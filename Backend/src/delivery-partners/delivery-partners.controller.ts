import { Controller, Post, Body, Get, Patch, Param, Delete, Query, UseGuards, Req, Logger } from '@nestjs/common';
...
export class DeliveryPartnersController {
    ...
    @UseGuards(DeliveryPartnerJwtGuard)
    @Patch('me/profile')
    async updateMyProfile(@Req() req: any, @Body() dto: UpdateDeliveryPartnerDto) {
    ...
    async register(@Body() registerDto: RegisterDeliveryPartnerDto) {
        this.logger.log(`Admin registering new delivery partner: ${registerDto.phone}`);
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles(UserRole.ADMIN)
    async updateStatus(@Param('id') id: string, @Body() dto: UpdateDeliveryPartnerDto) {
