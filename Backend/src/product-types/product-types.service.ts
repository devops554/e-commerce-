...
export class ProductTypesService {
  ...
  private async invalidateCache() {
    try {
      ...
      this.logger.warn(
        `Cache invalidation failed in ProductTypesService: ${error.message}`,
      );
    }
  }
  ...
  async create(
    ...
    try {
      const slug =
        createProductTypeDto.slug ||
        slugify(createProductTypeDto.name, { lower: true, strict: true });

      const existing = await this.productTypeModel.findOne({ slug }).exec();
      if (existing) {
        throw new ConflictException(
          `Product Type with slug "${slug}" already exists`,
        );
      }

      if (createProductTypeDto.order === undefined) {
        const lastType = await this.productTypeModel
          .findOne()
          .sort({ order: -1 })
          .exec();
      ...
      this.logger.error(
        `Failed to create product type: ${error.message}`,
        error.stack,
      );
  ...
  async findAll(query: {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch product types: ${error.message}`,
        error.stack,
      );
  ...
  async findOne(idOrSlug: string): Promise<ProductType> {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Failed to fetch product type ${idOrSlug}: ${error.message}`,
        error.stack,
      );
  ...
  async update(
    ...
    try {
      ...
      if (updateProductTypeDto.name && !updateProductTypeDto.slug) {
        updateData.slug = slugify(updateProductTypeDto.name, {
          lower: true,
          strict: true,
        });
      }
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Failed to update product type ${id}: ${error.message}`,
        error.stack,
      );
  ...
  async remove(id: string): Promise<void> {
    try {
      ...
      if (error instanceof HttpException) throw error;
      this.logger.error(
        `Failed to delete product type ${id}: ${error.message}`,
        error.stack,
      );
