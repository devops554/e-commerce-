...
export class CartsService {
    ...
    async getCart(userId: string) {
        try {
            ...
            this.logger.error(`Failed to get cart for user ${userId}: ${error.message}`, error.stack);
            throw new InternalServerErrorException('Failed to retrieve cart data');
        }
    }

    async addItem(userId: string, productId: string, variantId: string, quantity: number, title?: string, price?: number, image?: string) {
        try {
            this.validateObjectId(userId, 'user');
            ...
            const existingItem: any = await this.redisService.hget(cartKey, variantId);

            const newQuantity = (existingItem?.quantity || 0) + quantity;

            if (newQuantity <= 0) {
                await this.redisService.hdel(cartKey, variantId);
                this.logger.log(`Item ${variantId} removed from cart for user ${userId} (quantity <= 0)`);
            } else {
                const newItem = {
                    productId,
                    variantId,
                ...
                this.logger.log(`Item ${variantId} updated in cart for user ${userId} (qty: ${newQuantity})`);
            }

            return this.getCart(userId);
        } catch (error) {
            if (error instanceof HttpException) throw error;
            this.logger.error(`Failed to add item to cart for user ${userId}: ${error.message}`, error.stack);
    ...
    async removeItem(userId: string, variantId: string) {
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(`Failed to remove item from cart for user ${userId}: ${error.message}`, error.stack);
    ...
    async clearCart(userId: string) {
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(`Failed to clear cart for user ${userId}: ${error.message}`, error.stack);
            throw new InternalServerErrorException('Failed to clear cart data');
        }
    }

    async syncCart(userId: string, items: { productId: string; variantId: string; quantity: number, title: string, price: number, image: string }[]) {
        try {
            this.validateObjectId(userId, 'user');
            ...
            for (const item of items) {
                try {
                    ...
                    const existingItem: any = await this.redisService.hget(cartKey, item.variantId);
                    ...
                    this.logger.warn(`Skipping invalid item in sync for user ${userId}: ${itemError.message}`);
                }
            }

            this.logger.log(`Cart synced for user ${userId} with ${items.length} items`);
            return this.getCart(userId);
        } catch (error) {
            if (error instanceof HttpException) throw error;
            this.logger.error(`Failed to sync cart for user ${userId}: ${error.message}`, error.stack);
